    ...
    const auth = new JWT({
     ...
     // await sheet.addRows([
      ...
      for(let i = 1; i < 100; i++) {
        ...
        let username = await sheet.getCellByA1("A" + i)
        let password = await sheet.getCellByA1("B" + i)

        if(userInfo["username"] == username.value && userInfo["password"] == password.value) {
          arr.push({ token: { username: username.value}})
          break
        }
      }

      if(arr.length == 0) return res.status(400).json({message: "User not found"})
      return res.status(200).json({ token: jwt.sign(arr[0], 'zayinha')});
  }