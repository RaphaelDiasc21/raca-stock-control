  ...
  if(verifyToken(req.headers["authorization"])) {
    const auth = new JWT({
     ...
     // await sheet.addRows([
      ...
      for(let i = 2; i < 100; i++) {
        if(await sheet.getCellByA1("A" + i).value == null) {
          let productBody = JSON.parse(req.body)
          let produtoCell = await sheet.getCellByA1("A" + i)
          let quantidadeCell = await sheet.getCellByA1("B" + i)
          let updatedDateCell = await sheet.getCellByA1("C" + i)
          produtoCell.value = productBody['product']
          quantidadeCell.value = productBody['quantity']

          let updatedDateObj = new Date()
          updatedDateObj.setHours(updatedDateObj.getHours() - 3)
          updatedDateCell.value = String(
